...
unsigned char ide_print_error(unsigned int drive, unsigned char err) {
   ...
   cwrites("IDE:");
   if (err == 1) {cwrites("- Device Fault\n     "); err = 19;}
   else if (err == 2) {
      unsigned char st = ide_read(ide_devices[drive].Channel, ATA_REG_ERROR);
      if (st & ATA_ER_AMNF)   {cwrites("- No Address Mark Found\n     ");   err = 7;}
      if (st & ATA_ER_TK0NF)   {cwrites("- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_ABRT)   {cwrites("- Command Aborted\n     ");      err = 20;}
      if (st & ATA_ER_MCR)   {cwrites("- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_IDNF)   {cwrites("- ID mark not Found\n     ");      err = 21;}
      if (st & ATA_ER_MC)   {cwrites("- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_UNC)   {cwrites("- Uncorrectable Data Error\n     ");   err = 22;}
      if (st & ATA_ER_BBK)   {cwrites("- Bad Sectors\n     ");       err = 13;}
   } else  if (err == 3)           {cwrites("- Reads Nothing\n     "); err = 23;}
     else  if (err == 4)  {cwrites("- Write Protected\n     "); err = 8;}
   char buf[1024];
   sprint(buf, "- [%s %s] %s\n",
